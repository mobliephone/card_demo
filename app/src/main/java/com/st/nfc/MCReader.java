...
public class MCReader {
        ...
        if (auth) {
            ...
            for (int i = firstBlock; i < lastBlock; i++) {
                try {
                    ...
                    blocks.add(CMEApplication.byte2HexString(blockBytes));
                } catch (TagLostException e) {
                    throw e;
                } catch (IOException e) {
            ...
            if (!useAsKeyB) {
                if (isKeyBReadable(CMEApplication.hexStringToByteArray(
                        ret[last].substring(12, 20)))) {
                    ret[last] = CMEApplication.byte2HexString(key)
                ...
                if (ret[0].equals(NO_DATA)) {
                    ...
                    ret[last] = NO_KEY + ret[last].substring(12, 20)
                            + CMEApplication.byte2HexString(key);
                }
            }
        }
        return ret;
    ...
    public int buildNextKeyMapPart() {
        ...
        if (mKeysWithOrder != null && mLastSector != -1) {
            ...
            boolean autoReconnect = CMEApplication.getPreferences().getBoolean(
                    PreferencesActivity.Preference.AutoReconnect.toString(), false);
            // Get retry authentication option.
            boolean retryAuth = CMEApplication.getPreferences().getBoolean(
                    PreferencesActivity.Preference.RetryAuthentication.toString(), true);
    ...
    public HashMap<Integer, HashMap<Integer, Integer>> isWritableOnPositions(
        ...
        for (int i = 0; i < keyMap.size(); i++) {
            ...
            if (pos.containsKey(sector)) {
                ...
                byte[][] acMatrix = CMEApplication.acBytesToACMatrix(ac);
                if (acMatrix == null) {
                    ret.put(sector, null);
                    continue;
                }
                boolean isKeyBReadable = CMEApplication.isKeyBReadable(
                ...
                for (int block : pos.get(sector)) {
                    if ((block == 3 && sector <= 31)
                        ...
                        int acValue = CMEApplication.getOperationInfoForBlock(
                                acMatrix[0][3],
                                acMatrix[1][3],
                                acMatrix[2][3],
                                CMEApplication.Operations.WriteAC,
                                true, isKeyBReadable);
                        // Is key A writable? (If so, key B will be writable
                        // with the same key.)
                        int keyABValue = CMEApplication.getOperationInfoForBlock(
                                acMatrix[0][3],
                                acMatrix[1][3],
                                acMatrix[2][3],
                                CMEApplication.Operations.WriteKeyA,
                                true, isKeyBReadable);
                        ...
                        blockWithWriteInfo.put(
                                block, CMEApplication.getOperationInfoForBlock(
                                        acMatrix[0][acBitsForBlock],
                                        acMatrix[1][acBitsForBlock],
                                        acMatrix[2][acBitsForBlock],
                                        CMEApplication.Operations.Write,
    ...
    public boolean setKeyFile(File[] keyFiles, Context context) {
        ...
        for (File file : keyFiles) {
            String[] lines = CMEApplication.readFileLineByLine(file, false, context);
            if (lines != null) {
                for (String line : lines) {
                    if (!line.equals("") && line.length() == 12
                            && line.matches("[0-9A-Fa-f]+")) {
                        try {
                            keys.add(CMEApplication.hexStringToByteArray(line));
    ...
    public int getSectorCount() {
        boolean useCustomSectorCount = CMEApplication.getPreferences().getBoolean(
                PreferencesActivity.Preference.UseCustomSectorCount.toString(), false);
        if (useCustomSectorCount) {
            int customSectorCount = CMEApplication.getPreferences().getInt(
                    PreferencesActivity.Preference.CustomSectorCount.toString(), 16);
