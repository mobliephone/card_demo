...
/**
 * 一个简单的通用文件选择器，让用户选择文件
 * 给定目录。可选地，也可以删除文件或*创建新的
 *
 *
...
//        setContentView(R.layout.activity_nfc_file_chooser);
        mGroupOfFiles = (RadioGroup) findViewById(R.id.radioGroupFileChooser);
    }

    @Override
    public int getLayoutResId() {
        return R.layout.activity_nfc_file_chooser;
    }
    ...
    public void onStart() {
        ...
        if (intent.hasExtra(EXTRA_DIR)) {
            ...
            if (path.exists()) {
                ...
                // Path does not exist.
                Log.e(LOG_TAG, "Directory for FileChooserActivity does not exist.");
                setResult(1);
                finish();
                return;
            }
        } else {
            Log.d(LOG_TAG, "Directory for FileChooserActivity was not in intent.");
            setResult(2);
            finish();
            return;
        }
