...
public abstract class OpEnroll extends Thread implements PtGuiStateCallback {
    ...
    private IAddFingerCallback callback ;
    public OpEnroll(PtConnectionI conn, int fingerId , IAddFingerCallback callback ){
    ...
    public void run(){
        ...
        try{
            ...
            if(false){
                ...
                Log.e("logInfo","mIsoRawTemplate--->"+mIsoRawTemplate);
                ...
                Log.e("logInfo","aAlphaRawTemplate--->"+aAlphaRawTemplate);
            	...
            	if(mConn.verify(0,0,false,aAlphaInputBir,null
                        ,null,null,null
                        , PtConstants.PT_BIO_INFINITE_TIMEOUT,false,null
                        ,null,null)){
...
//            onDisplayMessage("Unable to set session cfg - " + e.getMessage());
            onDisplayMessage("无法打开指纹配置 - " + e.getMessage()+"！");
            throw e;
        }
    }
...
//            onDisplayMessage("Enrollment failed - " + e.getMessage());
            onDisplayMessage("初始化失败 - " + e.getMessage()+"！");
            throw e;
        }
...
//                                onDisplayMessage("Finger already enrolled as " + FingerId.NAMES[fingerId]);
                                onDisplayMessage("手指已经注册为：" + FingerId.NAMES[fingerId]+"！");
                                return false;
                            }
                        }
                    }
...
//            onDisplayMessage("testAndClean failed - " + e.getMessage());
            onDisplayMessage("测试和清除失败 - " + e.getMessage()+"！");
            return false;
        }
     ...
     * 注册指纹
     */
    private void addFinger(PtInputBir template , IAddFingerCallback callback) {
        try {
            ...
            Log.e("logInfo","addFinger--bir--DataFormat-->"+template.bir.getPtDataFormat());

            Log.e("logInfo","addFinger--form-->"+template.form);

            Log.e("logInfo","addFinger--bir--data-->"+template.bir.data);

            Log.e("logInfo","addFinger--bir--data--base64-->"+ Base64.encodeToString(template.bir.data,Base64.DEFAULT));

//            Log.e("logInfo","addFinger--slot-->"+slot);
//            Log.e("logInfo","fingerData-->"+fingerData);
//            Log.e("logInfo","mConn.getFingerData(slot)-->"+mConn.getFingerData(slot));
            if( null == callback ) return ;

            callback.onAddFinger( slot , mConn.getFingerData(slot));

        } catch (PtException e) {
//            onDisplayMessage("addFinger failed - " + e.getMessage());
            onDisplayMessage("指纹录入失败 - " + e.getMessage()+"！");
        }
